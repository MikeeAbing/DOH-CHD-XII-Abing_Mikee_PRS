  ...
  try {
    ...
    if (!empty($medicineDetailIds) && is_array($medicineDetailIds)) {
      ...
      for ($i = 0; $i < $size; $i++) {
        ...
        $queryMedicationHistory = "INSERT INTO `patient_medication_history`(`patient_visit_id`, `medicine_details_id`, `quantity`, `dosage`)
                                   VALUES (:visitId, :medicineDetailId, :quantity, :dosage)";
        $stmtDetails = $con->prepare($queryMedicationHistory);
        $stmtDetails->execute([
          ':visitId' => $lastInsertId,
          ':medicineDetailId' => $curMedicineDetailId,
...
include './config/sidebar.php';?>  
    ...
    <div class="col-lg-1 col-md-1 col-sm-6 col-xs-12">
      <label>&nbsp;</label>
      <button id="add_to_list" type="button" class="btn btn-success btn-sm btn-flat btn-block">
